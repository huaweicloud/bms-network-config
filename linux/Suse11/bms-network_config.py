...
REGEX = 'client is already running'
...
def bring_up_interface_hw(phy_cfg_total, bond_cfg_total, viface_cfg_total):
    ...
    for bond_cfg in bond_cfg_total:
        ...
        if status != 0 or re.search(REGEX, output):
            ...
            cmd = '/bin/ps ax|grep ' + bond_cfg["DEVICE"] + '|grep dhcp |grep -v grep'
            (status, output) = commands.getstatusoutput(cmd)
            if status == 0:
    ...
    for viface_cfg in viface_cfg_total:
        ...
        if status != 0 or re.search(REGEX, output):
            ...
            cmd = '/bin/ps ax|grep ' + viface_cfg["DEVICE"] + '|grep dhcp |grep -v grep'
            (status, output) = commands.getstatusoutput(cmd)
    ...
    cmd = '/bin/ps ax|grep dhcpcd |grep -v grep'
    (status, output) = commands.getstatusoutput(cmd)
    if status == 0:
        dhclient_infos = output.split('\n')
        for dhclient_info in dhclient_infos:
            (status, output) = commands.getstatusoutput('/bin/kill -s 9 ' + dhclient_info.split('?')[0])
            LOG.info("kill dhcpcd status is  %s, output is  %s" % (status, output))

    cmd = '/bin/ps ax|grep dhcpcd |grep -v grep'
    (status, output) = commands.getstatusoutput(cmd)
    if status == 0 and output:
        LOG.error("kill dhcpcd failed,try again later.")
        time.sleep(5)
        dhclient_infos = output.split('\n')
        for dhclient_info in dhclient_infos:
            (status, output) = commands.getstatusoutput('/bin/kill -s 9 ' + dhclient_info.split('?')[0])
            LOG.info("kill dhcpcd again, status is  %s, output is  %s" % (status, output))

    for viface_cfg in viface_cfg_total:
        (status, output) = commands.getstatusoutput('/sbin/ifup ' + viface_cfg["DEVICE"])
        LOG.info("vlanif dhcpcd status is %s, output is %s" % (status, output))
        time.sleep(5)

    for bond_cfg in bond_cfg_total:
        (status, output) = commands.getstatusoutput('dhcpcd ' + bond_cfg["DEVICE"])
        LOG.info("bond dhcpcd status is %s, output is %s" % (status, output))
        time.sleep(5)
