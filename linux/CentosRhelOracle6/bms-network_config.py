...
BOND_CONF = ['MACADDR', 'USERCTL', 'BONDING_MASTER',
             'ONBOOT','NM_CONTROLLED', 'BOOTPROTO',
             'BONDING_OPTS', 'DEVICE', 'TYPE', 'IPADDR', 'NETMASK', 'PERSISTENT_DHCLIENT']

VLAN_CONF = ['MACADDR', 'USERCTL', 'PHYSDEV',
             'ONBOOT', 'NM_CONTROLLED', 'BOOTPROTO',
             'DEVICE', 'TYPE', 'VLAN', 'IPADDR', 'NETMASK', 'PERSISTENT_DHCLIENT']
...
def _rename_interfaces_hw(renames, strict_present=True, strict_busy=True, current_info=None):
    ...
    for mac, new_name in renames:
        ...
        if not cur_name:
            if strict_present:
                LOG.error("[nic not present] Cannot rename mac=%s to %s" ", not available." % (mac, new_name))
            continue
...
def write_phy_conf_hw(ifphy_cfg_total):
    for ifphy_cfg in ifphy_cfg_total:
        try:
            open(path + ifphy_cfg['NAME'], 'w').close()
            phy_file = file(path + ifphy_cfg['NAME'], 'w+')
        except:
...
def write_bond_conf_hw(iface_cfg_total):
    for iface_cfg in iface_cfg_total:
        bond_links = iface_cfg['bond_links']
        for bond_link in bond_links:
            if not os.path.isfile(path + "ifcfg-" + bond_link):
                return False

        bond_to_phy = "%s=%s" % ('MASTER', iface_cfg['DEVICE'])
        slave_info = "SLAVE=yes"
        for bond_link in bond_links:
            ifcfg_path = path + "ifcfg-" + bond_link
            write_bond_to_phy_hw(ifcfg_path, bond_to_phy, slave_info)
        try:
            bond_file = file(path+iface_cfg['name'], 'w+')
        except:
            LOG.error("Can't open file %s" % iface_cfg['name'])
            return False
        ...
        try:
            ...
            LOG.error("Can't write file %s" % iface_cfg['name'])
            return False
        finally:
            bond_file.close()
        os.chmod(path+iface_cfg['name'], 0o644)
    return True
...
def write_vlanif_conf_hw(viface_cfg_total):
    for viface_cfg in viface_cfg_total:
        try:
            vif_file = file(path+viface_cfg['name'], 'w+')
        except:
            LOG.error("Can't open file %s" % (path+viface_cfg['name']))
            return False
        ...
        try:
            ...
            vif_file.close()
        os.chmod(path+viface_cfg['name'], 0o644)
    return True
...
def apply_phy_hw(links, netlist):
    ...
    for linkdic in links:
        if linkdic.get('type', None) == 'phy' and\
                ...
                linkdic.get('id', None):
            ifphy_cfg = {}
            ifphy_cfg['NAME'] = "%s%s" % ('ifcfg-', linkdic.get('id'))
            ifphy_cfg['DEVICE'] = linkdic.get('id')
            ifphy_cfg['MACADDR'] = linkdic.get('ethernet_mac_address')
            ...
            ifphy_cfg['ONBOOT'] = 'yes'
            ifphy_cfg['USERCTL'] = 'no'

            for networkdic in netlist:
                if networkdic.get('link', None) == linkdic.get('id'):
                    if networkdic.get('type') == 'ipv4_dhcp':
                        ifphy_cfg['BOOTPROTO'] = 'dhcp'
                        break
                    elif networkdic.get('type') == 'ipv4' and\
                            networkdic.get('netmask', None) and\
                            networkdic.get('ip_address', None):
                        ifphy_cfg['BOOTPROTO'] = 'static'
                        ifphy_cfg['NETMASk'] = networkdic.get('netmask')
                        ifphy_cfg['IPADDR'] = networkdic.get('ip_address')
                        break
                else:
                    continue
...
def apply_bond_hw(link_info, network_info):
    ...
    for linkdic in link_info:
        if linkdic.get('type', None) == 'bond' and\
            ...
            iface_cfg['ONBOOT'] = 'yes'
            iface_cfg['USERCTL'] = 'no'
            ...
            for networkdic in network_info:
                if networkdic.get('link', None) == linkdic.get('id'):
                    if networkdic.get('type') == 'ipv4':
                        ...
                        iface_cfg['BOOTPROTO'] = 'dhcp'
                        iface_cfg['PERSISTENT_DHCLIENT'] = '1'
                    iface_cfg['NM_CONTROLLED'] = 'no'
                    iface_cfg['ONBOOT'] = 'yes'
                    iface_cfg['USERCTL'] = 'no'
                else:
            ...
            iface_cfg['MACADDR'] = ethernet_mac_address
            iface_cfg['DEVICE'] = "%s%s" % ('bond', index)
            iface_cfg['name'] = "%s%s" % ('ifcfg-bond', index)
            index += 1
                    ...
                    for networkdic in network_info:
                        if networkdic.get('link', None) == linkdic.get('id'):
                            ...
                            if networkdic.get('type') == 'ipv4':
                                ...
                                viface_cfg['PERSISTENT_DHCLIENT'] = '1'

                            viface_cfg['DEVICE'] = "%s.%s" % (bonddic.get('DEVICE'),
                                                            linkdic.get('vlan_id'))
                            viface_cfg['MACADDR'] = linkdic.get('vlan_mac_address')
                            viface_cfg['PHYSDEV'] = bonddic.get('DEVICE')
                            viface_cfg['VLAN'] = 'yes'
                            viface_cfg['TYPE'] = 'Ethernet'
                            viface_cfg['NM_CONTROLLED'] = bonddic.get('NM_CONTROLLED')
                            viface_cfg['ONBOOT'] = bonddic.get('ONBOOT')
                            viface_cfg['USERCTL'] = bonddic.get('USERCTL')
                            viface_cfg['name'] = "%s.%s" % (bonddic.get('name'), vlan_id)
...
def render_80_persistent_rules_hw(ifphy_cfg_total):
    ...
    try:
        ...
        LOG.error("Open file Error:%s" % persistant_file)
        raise IOError
        return False
    ...
    try:
        ...
        LOG.error("Can't open file %s" % (persistant_file))
        return False
    ...
    try:
        ...
        LOG.error("Can't write file %s" % (persistant_file))
        return False
    finally:
        per_file.close()
    ...
    try:
        ...
        LOG.error("rename interfaces err.")
    return True
...
def bring_up_interface_hw(bond_cfg_total, viface_cfg_total):

    LOG.info("bring up bond start")

    for bond_cfg in bond_cfg_total:
        (status, output) = commands.getstatusoutput('/sbin/ifup ' + bond_cfg["DEVICE"])
        LOG.info("bring up bond status is %s,output is %s" % (status, output))
        if status != 0 or re.search(REGEX, output):
            LOG.error("Can't ifup interface %s,try again" % (bond_cfg["DEVICE"]))
            (status, output) = commands.getstatusoutput('/bin/ps ax|grep dhclient|grep -v grep|grep '
			                                            + bond_cfg["DEVICE"])

            LOG.info("grep bond dhclient status is %s,output is %s" % (status, output))
            if status == 0:
                dhclient_infos = output.split('\n')
                for dhclient_info in dhclient_infos:
                    commands.getstatusoutput('/bin/kill -s 9 ' + dhclient_info.split('?')[0])
                    LOG.info("try to kill bond dhclient. pid is:%s" % dhclient_info.split('?')[0])
            (status, output) = commands.getstatusoutput('/sbin/ifup ' + bond_cfg["DEVICE"])
            LOG.info("bring up bond status is %s,output is %s" % (status, output))
        time.sleep(10)
    ...
    for bond_cfg in bond_cfg_total:
        (status, output) = commands.getstatusoutput('/sbin/ifconfig ' + bond_cfg["DEVICE"]+' promisc')
        LOG.info("set bond promisc status is %s,output is %s" % (status, output))
    LOG.info("set bond promisc finish.")

    for viface_cfg in viface_cfg_total:
        (status, output) = commands.getstatusoutput('/sbin/ifup ' + viface_cfg["DEVICE"])
        LOG.info("bring up viface status is %s,output is %s" % (status, output))
        if status != 0 or re.search(REGEX, output):
            LOG.error("Can't ifup interface %s,try again" % (viface_cfg["DEVICE"]))
            (status, output) = commands.getstatusoutput('/bin/ps ax|grep dhclient|grep -v grep|'
                                                        'grep '+viface_cfg["DEVICE"])
            LOG.info("grep viface dhclient status is %s,output is %s" % (status, output))
            if status == 0 and output:
                dhclient_infos = output.split('\n')
                for dhclient_info in dhclient_infos:
                    commands.getstatusoutput('kill -s 9 ' + dhclient_info.split('?')[0])
                    LOG.info("try to kill viface dhclient.pid is:%s" % dhclient_info.split('?')[0])
            (status, output) = commands.getstatusoutput('/sbin/ifup ' + viface_cfg["DEVICE"])
    ...
    for bond_cfg in bond_cfg_total:
        (status, output) = commands.getstatusoutput('/sbin/ifup ' + bond_cfg["DEVICE"])
        LOG.info("bring up bond again, status is %s,output is %s" % (status, output))
        (status, output) = commands.getstatusoutput('/sbin/ifconfig ' + bond_cfg["DEVICE"]+' promisc')
        LOG.info("set bond promisc status is %s,output is %s" % (status, output))
        time.sleep(5)
...
def is_hostname_exist():
    f1 = file('/etc/sysconfig/network', mode='r')
    line = f1.readline()
    while line:
...
def write_hostname_hw(hostname_cfg):
    if is_hostname_exist():
        cmd = ('sed' + ' -i' + ' s/HOSTNAME=.*/HOSTNAME=' + hostname_cfg['HOSTNAME']+ '/g' + ' /etc/sysconfig/network')
        (status, output) = commands.getstatusoutput(cmd)
        if status:
            LOG.error("Failed to set hostname,because:%s", output)
            return False
    else:
        hostname_file = open('/etc/sysconfig/network', mode='a')
        hostname_file.write('\n' + 'HOSTNAME' + '=' + hostname_cfg['HOSTNAME'] + '\n')
        hostname_file.close()
    try:
        subp_hw(['hostname', hostname_cfg['HOSTNAME']])
    except IOError:
...
def process_network_hw():
    ...
    ret = bring_up_interface_hw(ifbond_cfg_total, ifvif_cfg_total)
    if not ret:
